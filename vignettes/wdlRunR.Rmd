...
This package provides a simple api to the [Broad cromwell workflow
engine](https://github.com/broadinstitute/cromwell).
...
```{r}
library(wdlRunR)
cromwell_jar = getCromwellJar(cromwell_version='36')
...
Cromwell should now be running in the background and we can verify by
checking for processes with "cromwell" in the name.
...
The simplest example (and one that runs in the time needed for a
demo!) is a "Hello, WDL" workflow. Right now, we simply describe a WDL
workflow as text or as a text file (see below).
...
At this point, checking `cromwellQuery()` will show running jobs (if
done within 60 seconds, of course). Limiting to "Running" jobs is also
easy.
